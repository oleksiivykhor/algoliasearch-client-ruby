...
module Helpers
  ...
  def check_array(object)
    raise AlgoliaV2::AlgoliaError, 'argument must be an array of objects' unless object.is_a?(Array)
  end
  ...
  def check_object(object, in_array = false)
    case object
    when Array
      raise AlgoliaV2::AlgoliaError, in_array ? 'argument must be an array of objects' : 'argument must not be an array'
    when String, Integer, Float, TrueClass, FalseClass, NilClass
      raise AlgoliaV2::AlgoliaError, "argument must be an #{'array of' if in_array} object, got: #{object.inspect}"
    end
  end
  ...
  def get_object_id(object, object_id = nil)
    ...
    raise AlgoliaV2::AlgoliaError, "Missing 'objectID'" if object_id.nil?
    object_id
  end
